...
class Generator:
    ...
    def _extract_gen_code(self, response: str) -> str:
        """
        从文本中提取第一个 <cuda>...</cuda> 之间的代码片段。
        如果没有匹配，则返回空字符串。
        """
        ...
        if matches:
            code = matches[0].strip()  # 取第一个匹配并去掉首尾空白
            return code
        else:
            return ""

    def run(self, llm: BaseModel, prompt: str):
        status, response = llm.request(self.system_prompt, prompt)
        code = self._extract_gen_code(response)
        return status, code
        code = """
